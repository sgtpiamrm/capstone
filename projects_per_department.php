...
if (isset($_GET['id'])) {
    // Query to fetch department details based on the given ID
    $qry = $conn->query("SELECT * FROM department_list where `status` = 1 and id = '{$_GET['id']}' ");

    // Check if any department record is found
    if ($qry->num_rows > 0) {
        ...
        // If no department is found, display an alert and redirect to the home page
    ...
    <div class="col-12">
        <div class="card card-outline card-primary shadow rounded-0">
            <div class="card-body rounded-0">
                <!-- Display the department title and description -->
                ...
                <?php
                // Set the department ID from the URL and initialize pagination parameters
                ...
                $wheredid = " where department_id = '{$id}' "; // Filter by department ID
                
                // Query to get students associated with the department's curriculum
                ...
                // Count the total number of archives for the selected department's curriculum
